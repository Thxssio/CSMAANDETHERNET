...
# ---------------- Função de Simulação com Recuo Binário Exponencial ---------------- #
def simulate_binexp(taxa_geracao, n_est=10, atraso_a=0.02, rodadas=100):
    ...
    for _ in range(rodadas):
        ...
        tx_cnt = [0] * n_est
        colin = [0] * n_est
        BE = [0] * n_est  # Exponente de backoff
        K = [0] * n_est   # Tentativas
        tx_espera = [0] * n_est
            ...
            for j in range(n_est):
                ...
                if tx_cnt[j] > 0:
                    if colin[j] == 1:
                        ...
                        K[j] += 1
                        if K[j] > 16:
                            # falha total, descarta o quadro
                            K[j] = 0
                            BE[j] = 0
                            continue

                        BE[j] = min(BE[j] + 1, 10)
                        backoff_slots = random.randint(0, 2**BE[j] - 1)
                        tx_espera[j] = backoff_slots
                        tx_fila[j] += 1
                    else:
                        tx_cnt[j] -= 1
                        if tx_cnt[j] == 0:
...
# ---------------- Versão SEM backoff exponencial ---------------- #
def simulate_nobackoff(taxa_geracao, n_est=10, atraso_a=0.02, rodadas=100):
    t_sim = int(sim_time / dt_sim)
    atraso = int(atraso_a * L / R / dt_sim)
    entregues = colididos = bloqueios = gerados = 0
...
def main():
    ...
    for taxa in tqdm(taxas_quadro, desc="Simulando (Backoff Exponencial)", unit="ponto"):
        gerados, entregues, colididos, bloqueios = simulate_binexp(taxa, n_est=n_est, rodadas=rodadas)
        g = gerados * L / sim_time / R
        s = entregues * L / sim_time / R
        g_list_binexp.append(g)
        s_list_binexp.append(s)
        tg_bps_binexp.append(gerados * L / sim_time)
        td_bps_binexp.append(entregues * L / sim_time)

    # Simulação SEM backoff exponencial
    g_list_noback = []
    s_list_noback = []
    tg_bps_noback = []
    td_bps_noback = []

    for taxa in tqdm(taxas_quadro, desc="Simulando (Sem Backoff)", unit="ponto"):
        gerados, entregues, colididos, bloqueios = simulate_nobackoff(taxa, n_est=n_est, rodadas=rodadas)
        g = gerados * L / sim_time / R
        s = entregues * L / sim_time / R
        g_list_noback.append(g)
        s_list_noback.append(s)
        tg_bps_noback.append(gerados * L / sim_time)
        td_bps_noback.append(entregues * L / sim_time)
        
    # Gráfico Eficiência vs G
    plt.figure()
    plt.plot(g_list_binexp, s_list_binexp, 'ro-', label="Backoff Exponencial")
    plt.plot(g_list_noback, s_list_noback, 'bs--', label="Sem Backoff")
    
    plt.figure()
    plt.plot(g_list_binexp, s_list_binexp, 'ro-', label="Backoff Exponencial")
    plt.plot(g_list_noback, s_list_noback, 'bs--', label="Sem Backoff")
    max_idx = np.argmax(s_list_binexp)
    plt.plot(g_list_binexp[max_idx], s_list_binexp[max_idx], 'ko', label="Pico simulação")
    ...
    plt.figure()
    plt.plot(tg_bps_binexp, td_bps_binexp, 'ro-', label="Backoff Exponencial")
    plt.plot(tg_bps_noback, td_bps_noback, 'bs--', label="Sem Backoff")
    
    plt.figure()
    plt.plot(tg_bps_binexp, td_bps_binexp, 'ro-', label="Backoff Exponencial")
    plt.plot(tg_bps_noback, td_bps_noback, 'bs--', label="Sem Backoff")
    max_idx_th = np.argmax(td_bps_binexp)
    plt.plot(tg_bps_binexp[max_idx_th], td_bps_binexp[max_idx_th], 'ko', label="Pico simulação")
